{{-- <div class="card-item unit-card">
    <div class="card-item__content">
        ...
        <div class="card-item__header">
            @if ($unit->project)
                @if ($unit->project->developer)
                    <a href="{{ route('front.developers.show', ['id' => $unit->project->developer->id, 'slug' => $unit->project->developer->slug]) }}"
                        class="dev-logo">
                        ...
                        @endif
                    @empty
                        <img onerror="this.remove()" src="{{ URL::asset('front/img/logo_1.png') }}"
        ...
        @empty
            ...
            <img class="unit-img placeholder-img" onerror="this.remove()"
                src="{{ URL::asset('front/img/logo_1.png') }}" alt="{{ $unit->title }}"
                itemprop="image">
        @endforelse
    </div>
    ...
    <div class="commercial-tag">
        <a
            href="{{ route('front.properties', ['offering_types[]' => $unit->offering_type_id]) }}">{{ $unit->offering_type }}</a>
    </div>
</div>
...
<div class="card-item__footer">
    <h4 class="card-title text-truncate" title="{{ $unit->title }}">
        <a
            href="{{ route('front.singleUnit', ['id' => $unit->id, 'title' => str_slug($unit->default_title)]) }}">{{ $unit->title }}</a>
    </h4>

    <ul class="card-info">
        @if ($unit->purpose || $unit->purpose_type)
            <li>
...
<a class="site-btn d-block text-center stretched-link"
    href="{{ route('front.singleUnit', ['id' => $unit->id, 'title' => str_slug($unit->default_title)]) }}"
...
<div class="feature-item wow zoomIn" data-wow-duration="1s" data-wow-offset="300">
    <!-- open feature-item -->
    <div class="feature-pic">
        ...
        @empty
            ...
            <img class="" onerror="this.remove()"
                src="{{ asset('front/img/logo_1.png') }}" alt="{{ $unit->title }}" itemprop="image">
        @endforelse
    </a>
...
<div class="feature-info">
    ...
    <div class="price-location">
        <p class="center">
            <i class="fa fa-map-marker"></i> {{ $unit->city->name }}
        </p>
        @if ($unit->price)
            <strong class="propetry-price">{{ $unit->price }} EGP</strong>
        @endif
    </div>
    <div class="property-info">
        ...
        <div class="room-info">
            ...
            <div class="left">
                <!-- open left -->
                @if ($unit->area)
                    <p><i class="fa fa-th-large"></i> <strong>{{ $unit->area }} {{ $unit->area_unit }}
                        </strong>m<sup>2</sup></p>
                @endif
                @if ($unit->bedroom)
                    <p><i class="fa fa-bed"></i> {{ $unit->bedroom }} Bedrooms</p>
                @endif
            </div>
            <!-- close left -->
            <div class="right">
                ...
                @if ($unit->bathroom)
                    <p><i class="fa fa-bath"></i> {{ $unit->bathroom }} Bathrooms</p>
                @endif
            </div>
            <!-- close right -->
        </div>
