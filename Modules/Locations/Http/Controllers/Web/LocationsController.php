...
class LocationsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if (isset($request->id) && !is_null($request->id)) {
                $location = Location::active()->where('id', $request->id)->with(
                    'children',
                    'parent'
                )->first();

                $breadcrumbs =  (array)$this->getParentNames($location);

                $breadcrumbs = array_reverse($breadcrumbs, true);
                return view('locations::locations.' . $blade_name, ['breadcrumbs' => $breadcrumbs]);
    ...
    /**
     * Create locations parents name
     * @return Response
     */
    public function getParentNames($location)
    {
        if (is_null($location->value)) {
            # code...
        } else {
            array_push($this->arrayData, ['id' => $location->id, 'name' => $location->value]);
        }
        if ($location->parent) {
            return $this->getParentNames($location->parent);
        } else {
