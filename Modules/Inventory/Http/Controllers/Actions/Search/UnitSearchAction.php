...
class UnitSearchAction
{
    public function execute($data)
    {
        // Search units
        $units = new IUnit();
        if (isset($data['region_id']) && !empty($data['region_id']) && is_array($data['region_id']) && !empty($data['region_id'][0]) ) {
        ...
        if (isset($data['q']) && !empty($data['q'])) {
            ...
            $units = $units->where(function ($query) use ($data) {
                ...
                ->orWhereHas('translations', function ($translations) use ($data) {
        ...
        if (isset($data['bedrooms'])&& !empty($data['bedrooms'][0]) ) {
            $units = $units->whereIn('i_bedroom_id', $data['bedrooms']);
        }
        if (isset($data['bathrooms'])&& !empty($data['bedrooms'][0])) {
            $units = $units->whereIn('i_bathroom_id', $data['bathrooms']);
        }
